...
namespace KinectServer
{
    ...
    public delegate void ObjectListChangedHandler(List<float> list);
    public class KinectServer
    {
        ...
        public void GetLatestFrame(List<List<byte>> lFramesRGB, List<List<Single>> lFramesVerts, List<List<Body>> lFramesBody, List<List<Single>> lFramesFace, List<List<Single>> lFramesObject)
        {
            lFramesRGB.Clear();
            lFramesVerts.Clear();
            lFramesBody.Clear();
            ...
            lock (oFrameRequestLock)
            {
                ...
                lock (oClientSocketLock)
                {
                    for (int i = 0; i < lClientSockets.Count; i++)
                    {
                        ...
                        lFramesFace.Add(new List<Single>(lClientSockets[i].lFaces));                        
                        lFramesObject.Add(new List<Single>(lClientSockets[i].lObject));
                    }
                }
            }
        }
        ...
        public void sendim(int i, List<float> lPoint) //포인트 좌표 보낸당....
        {
            lock (oClientSocketLock) // ??? 요것도 나중에 봐야하나;;
            {

                int num = lPoint.Count / 10;
                
                if(num != 0) //예외처리 필용
                {
                lClientSockets[i].Sendimp(num, lPoint); //보내장
                }
            }
        }
