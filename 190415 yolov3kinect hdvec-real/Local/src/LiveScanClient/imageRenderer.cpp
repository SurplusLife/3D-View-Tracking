...
HRESULT ImageRenderer::EnsureResources()
{
    ...
    if (NULL == m_pRenderTarget)
    {
		...
		m_pRenderTarget->CreateSolidColorBrush(D2D1::ColorF(0.0f, 1.0f, 0.0f), &grenc);
		m_pRenderTarget->CreateSolidColorBrush(D2D1::ColorF(0.0f, 1.0f, 1.0f), &acuc);
		m_pRenderTarget->CreateSolidColorBrush(D2D1::ColorF(1.0f, 1.0f, 0.0f), &yellc);
		m_pRenderTarget->CreateSolidColorBrush(D2D1::ColorF(0.58f, 0.0f, 0.82f), &purpc);
...
HRESULT ImageRenderer::Drawp(BYTE* pImage, unsigned long cbImage, Point3f *view,int hnum)
{
	// incorrectly sized image data passed in
	if (cbImage < ((m_sourceHeight - 1) * m_sourceStride) + (m_sourceWidth * 4))
	{
	...
	{ 
		   ...
		   { 
			...
			tempPoint.x = view[i].X;
			tempPoint.y = view[i].Y;
	
			D2D1_ELLIPSE ellipse = D2D1::Ellipse(tempPoint, 140.0f, 140.0f); // 타원 선원

				//	m_pRenderTarget->FillEllipse(ellipse, m_pBrushJointInferred); // 꽉찬 타원 생성
			m_pRenderTarget->DrawEllipse(ellipse, grenc, 10.0f); // 타원 외곽선만 생성
		   }
		   else if (view[i].Z == 2)
		   {
			   ...
			   D2D1_ELLIPSE ellipse = D2D1::Ellipse(tempPoint, 140.0f, 140.0f); // 타원 선원
																				...
																				//	m_pRenderTarget->FillEllipse(ellipse, m_pBrushJointInferred); // 꽉찬 타원 생성
			   m_pRenderTarget->DrawEllipse(ellipse, acuc, 10.0f); // 타원 외곽선만 생성
		   }
		   else if (view[i].Z == 3)
		   {
			   ...
			   D2D1_ELLIPSE ellipse = D2D1::Ellipse(tempPoint, 140.0f, 140.0f); // 타원 선원
																				...
																				//	m_pRenderTarget->FillEllipse(ellipse, m_pBrushJointInferred); // 꽉찬 타원 생성
			   m_pRenderTarget->DrawEllipse(ellipse, yellc, 10.0f); // 타원 외곽선만 생성
		   }
		   else
		   {
			   ...
			   D2D1_ELLIPSE ellipse = D2D1::Ellipse(tempPoint, 140.0f, 140.0f); // 타원 선원
																				...
																				//	m_pRenderTarget->FillEllipse(ellipse, m_pBrushJointInferred); // 꽉찬 타원 생성
			   m_pRenderTarget->DrawEllipse(ellipse, purpc, 10.0f); // 타원 외곽선만 생성
		   }
...
void ImageRenderer::DrawBody(Body &body)
{
	...
	*/
	...
	for (unsigned int i = 0; i < body.vJoints.size(); i++)   // 관절 표시하는 부분
	{
		D2D1_POINT_2F tempPoint;
		tempPoint.x = body.vJointsInColorSpace[i].X;
		tempPoint.y = body.vJointsInColorSpace[i].Y;

		D2D1_ELLIPSE ellipse = D2D1::Ellipse(tempPoint, 50.0f, 50.0f); // 타원 선원

		if (body.vJoints[i].TrackingState == TrackingState_Inferred)
		{
		//	m_pRenderTarget->FillEllipse(ellipse, m_pBrushJointInferred); // 꽉찬 타원 생성
			m_pRenderTarget->DrawEllipse(ellipse, m_pBrushJointInferred, 2.0f); // 타원 외곽선만 생성
		}
		else if (body.vJoints[i].TrackingState == TrackingState_Tracked)
		{
		//	m_pRenderTarget->FillEllipse(ellipse, m_pBrushJointTracked);
			m_pRenderTarget->DrawEllipse(ellipse, m_pBrushJointTracked, 2.0f);
		}
	}
}
